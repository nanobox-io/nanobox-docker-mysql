VERSION=$1
echo running tests for $VERSION of mysql
UUID=$(cat /proc/sys/kernel/random/uuid)
PAYLOAD='{"boxfile":{"name":"database1","stability":"stable","version":'${VERSION}'},"logtap_host":"127.0.0.1","platform":"local","uid":"mysql1"}'
echo boxfile: "$PAYLOAD"

pass "unable to start the $VERSION container" docker run --privileged=true -d --name $UUID nanobox/mysql:$VERSION
